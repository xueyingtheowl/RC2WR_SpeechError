...
```{r}
#reading in long data from cleaned CSV file
analysis_dataset_long = read.table("/Users/wubabear/Desktop/tpa_data_long_cleaned_xw.csv",
                                   sep = ",",
                                   header = TRUE,
...
```{r}
#making R treat categorical variables the desired way
...
```{r}
#releveling so that "Recall" is the reference level for regression analysis
...
#modeling without repeated measures, multiplying by 2 to eliminate issue with ".5" values 
# + ent_focus + phon_focus + lang_expose + bilingual + age + gender
#fitting model for just Experiment 2, adjusting for Order
...
```
```{r}
#fitting model for both Experiments together, adjusting for Order
# + ent_focus + phon_focus + lang_expose + bilingual + age + gender
...
```
```{r}
#modeling with random effect for participant
