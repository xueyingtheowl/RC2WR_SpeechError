...
#reading Excel file
order1 = read_xlsx("/Users/wubabear/Desktop/sum_copy.xlsx",
                   sheet = "sum",
...
```
```{r}
order2 = read_xlsx("/Users/wubabear/Desktop/sum_copy.xlsx",
                   sheet = "sum",
                   col_names = TRUE,
                   na = c("", "null"),
...
```
```{r}
#checking discrepancies in column names...nothing major, just spacing characters
colnames(order1)[9:56] == colnames(order2)[57:104]
colnames(order2)[9:56] == colnames(order1)[57:104]
```
```{r}
#reordering columns of order2 to line up with column order from order1
...
```
```{r}
#vertically merging these together to create one big analysis dataset
...
#setting up column names for long data, and reformatting
...
```{r}
#making blank data frame for conversion to long format
...
```{r}
#getting list of ids
# unique() returns us a dataframe that duplicated data is being removed, here if there is a repeat in subject_name then that vector will be removed. Although our data does not have repeated subject_name doing this will double proof the data.
...
write.table(analysis_dataset_long,
            "/Users/wubabear/Desktop/tpa_data_long_cleaned_xw.csv",
            sep = ",",
            row.names = FALSE,
            quote = FALSE)
```
...
```{r}
table = read.csv("/Users/wubabear/Desktop/tpa_data_long_cleaned_xw.csv")
glimpse(table)
```
